...
public class RKLogEventManager {
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        //log event ads  to firebase,facebook
        logEventWithAds(context, (float) adValue.getValueMicros(), adValue.getPrecisionType(), adUnitId, mediationAdapterClassName, RKAdConfig.PROVIDER_ADMOB);

        //log event value adMod to adjust
        RKAdjust.pushTrackEventAdmob(adValue);

        //add tracking custom appsFlyer (custom event af_ad_revenue mkt request co the bo)
        RKAppsflyer.getInstance().pushTrackEventAdmob(adValue, adUnitId, adType);
        RKAppsflyer.getInstance().onTrackingAfAdRevenue(adValue.getValueMicros() * 1.0f, Currency.getInstance("USD").getDisplayName(), adUnitId, adType.toString());

        // Log revenue Facebook
    ...
    public static void logPaidAdImpression(Context context, MaxAd adValue, AdType adType) {
        ...
        logEventWithMaxAdsCustom(context, adValue);
        RKAdjust.pushTrackEventApplovin(adValue, context);
        RKAppsflyer.getInstance().pushTrackEventApplovin(adValue, adType);
    }
    ...
    private static void logEventWithMaxAdsCustom(Context context, MaxAd impressionData) {
        double revenue = impressionData.getRevenue(); // In USD
        Bundle params = new Bundle();
        params.putString(FirebaseAnalytics.Param.AD_PLATFORM, "AppLovin");
        params.putString(FirebaseAnalytics.Param.AD_SOURCE, impressionData.getNetworkName());
        ...
        params.putDouble(FirebaseAnalytics.Param.VALUE, revenue);
        params.putString(FirebaseAnalytics.Param.CURRENCY, "USD"); // All Applovin revenue is sent in USD
        FirebaseAnalytics.getInstance(context).logEvent("ad_impression_max", params);
    }
    ...
    private static void logEventWithAds(Context context, float revenue, int precision, String adUnitId, String network, int mediationProvider) {
        Log.d(TAG, String.format("Paid event of value %.0f microcents in currency USD of precision %s%n occurred for ad unit %s from ad network %s.mediation provider: %s%n", revenue, precision, adUnitId, network, mediationProvider));
    ...
    private static void logPaidAdImpressionValue(Context context, double value, int precision, String adunitid, String network, int mediationProvider) {
        ...
        params.putString("currency", "USD");
        params.putString("ad_platform", "Max");
        params.putInt("precision", precision);
        params.putString("currency", "USD");
        params.putString("country_code", AppLovinSdk.getInstance(context).getConfiguration().getCountryCode());
        params.putString("ad_unit_name", adunitid);
        params.putString("ad_source", network);


        RKAdjust.logPaidAdImpressionValue(value, "USD");

        //cai nay la goc, phan cua rocket la custom event co the xoa
        FirebaseAnalyticsUtil.logPaidAdImpressionValue(context, params, mediationProvider);
        FirebaseAnalyticsUtil.logEventAdsImpressionRocket(context, params, mediationProvider);
    ...
    public static void pushTrackEventApplovin(MaxAd ad, Context context) {
        RKAdjust.pushTrackEventApplovin(ad, context);
    }
}
