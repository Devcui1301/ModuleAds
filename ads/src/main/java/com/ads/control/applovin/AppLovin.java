...
//    private final Map<String, AppOpenAd> appOpenAdMap = new HashMap<>();
    ...
    public void loadSplashInterstitialAds(final Context context, String id, long timeOut, long timeDelay, AppLovinCallback adListener) {
        ...
        interstitialSplash.setListener(new MaxAdListener() {
            ...
            public void onAdClicked(MaxAd ad) {
                ...
                RKLogEventManager.logClickAdsEvent(context, ad.getAdUnitId());
            }
    ...
    public void onShowSplash(Activity activity, AppLovinCallback adListener) {
        ...
        interstitialSplash.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression(context,ad, AdType.INTERSTITIAL));
        interstitialSplash.setListener(new MaxAdListener() {
            @Override
            public void onAdLoaded(MaxAd ad) {
            ...
            @Override
            public void onAdClicked(MaxAd ad) {
                RKLogEventManager.logClickAdsEvent(context, interstitialSplash.getAdUnitId());
                if (adListener != null) {
                    adListener.onAdClicked();
                }
                if (disableAdResumeWhenClickAds)
    ...
    public void showInterstitialAdByTimes(final Context context, MaxInterstitialAd interstitialAd, final AdCallback callback, final boolean shouldReloadAds) {
        ...
        interstitialAd.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression(context,ad, AdType.INTERSTITIAL));
        interstitialAd.setListener(new MaxAdListener() {
            @Override
            public void onAdLoaded(MaxAd ad) {
...
//                new Handler().postDelayed(interstitialAd::showAd, 800);
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        interstitialAd.showAd(MaxPlacement.getNamePlacement(interstitialAd.getAdUnitId()));
                    }
                }, 800);
            }
            currentClicked = 0;
    ...
    private void loadBanner(final Activity mActivity, String id, final FrameLayout adContainer, final ShimmerFrameLayout containerShimmer) {
        ...
        adView.setPlacement(new MaxPlacement().getNamePlacement(id));
        adView.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression(mActivity, ad, AdType.BANNER));
    ...
    private void loadBanner(final Activity mActivity, String id, final FrameLayout adContainer, final ShimmerFrameLayout containerShimmer, final AdCallback adCallback) {
        ...
        containerShimmer.startShimmer();
        MaxAdView adView = new MaxAdView(id, mActivity);
        adView.setPlacement(MaxPlacement.getNamePlacement(id));
        adView.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression(mActivity, ad, AdType.BANNER));
        int width = ViewGroup.LayoutParams.MATCH_PARENT;
        // Banner height on phones and tablets is 50 and 90, respectively
        int heightPx = mActivity.getResources().getDimensionPixelSize(R.dimen.banner_height);
    ...
    public void loadNativeAd(Activity activity, ShimmerFrameLayout containerShimmer, FrameLayout nativeAdLayout, String id, int layoutCustomNative) {
        ...
        nativeAdLoader.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression(activity, ad, AdType.NATIVE));
        nativeAdLoader.setPlacement(MaxPlacement.getNamePlacement(id));
    ...
    public void showRewardAd(Activity activity, MaxRewardedAd maxRewardedAd, AppLovinCallback callback) {
        if (maxRewardedAd.isReady()) {
            maxRewardedAd.setRevenueListener(ad -> RKLogEventManager.logPaidAdImpression( activity,ad, AdType.REWARDED));
            ...
            maxRewardedAd.showAd(MaxPlacement.getNamePlacement(maxRewardedAd.getAdUnitId()));
        } else {
